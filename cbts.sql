...
--
-- Host: localhost
-- Generation Time: 2016-02-02 15:33:29
-- 服务器版本： 5.6.10
-- PHP Version: 5.3.28
...
CREATE TABLE IF NOT EXISTS `choc_issue` (
  ...
  `resolve` tinyint(4) NOT NULL DEFAULT '0' COMMENT '0:未解决;1已解决',
  PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `choc_repos` (
  ...
  `task_id` varchar(100) DEFAULT NULL COMMENT '关联的任务',
...
CREATE TABLE IF NOT EXISTS `choc_test` (
  ...
  `rank` tinyint(3) NOT NULL DEFAULT '0' COMMENT '处理状态:0待测,1测试中,-3不通过,3通过',
  `jieduan` tinyint(3) NOT NULL DEFAULT '0' COMMENT '阶段:0待测,1:在测,3:通过,-3不通过',
  ...
  KEY `status` (`status`),
  KEY `rank` (`rank`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='提测代码库' AUTO_INCREMENT=6 ;
