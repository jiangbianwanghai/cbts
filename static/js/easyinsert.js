   ...
   if ( allowed ) {
    ...
    $Clicker.bind("click", function() {
     ...
     if ( !isMax ) {//没有达到上限才允许添加
      var $Item = $("<"+ o.wrap +" class=\"control-group\">").appendTo($container);
      $.each(arr[0], function(i) {
       switch ( arr[1][i] ) {
        ...
        case "custom"://自定义内容，支持html
         ...
         break;
        case "label":
         var label = "<label class=\"control-label\" for=option" + (len + 1)+">Tower地址"+ (len + 1) +"：</label>";
         $Item.append( label );
         break;
        default://默认是input
      ...
      len = $Item.length;
      if ( len > 1 ) {
       $Item.last().children('.controls').append(oo.remove);
       if ( len === 2 ) {//超过一组时，为第一组添加"移除"按钮
        $Item.first().children('.controls').append(oo.remove);
        
       }
      }
      $Item.find('a').hide();
      $Item.last().find('a').show();
      $Item.find(".remove").click(function(){
       ...
       $container.children(o.wrap).last().find('a').show();
       //取消"移除"按钮的默认动作
       return false;
      });
     }
